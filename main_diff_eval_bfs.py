...
class Args_final_eval:
    def __init__(self):
		...
		"""
        self.parser.add_argument(
            "--Nt_read", default=40, help="Which Nt model we need to read"
        )
        self.parser.add_argument("--use_best", default=True)
        """
		reading the diffusion model
		"""
        self.parser.add_argument(
            "--Nepoch_read", default=2, help="Which epoch model we need to read"
        )
    ...
    def update_args(self):
        ...
        args.diff_args_txt = os.path.join(
            args.bfs_dynamic_folder, "diffusion_folder", "logging", "args.txt"
        )

        # output dataset
        args.experiment_path = os.path.join(
            args.bfs_dynamic_folder, "diffusion_folder", "experiment_final"
        )
...
if __name__ == "__main__":
    ...
    args_seq = read_args_txt(Args_seq(), args_final.seq_args_txt)
    args_diff = read_args_txt(Args_diff(), args_final.diff_args_txt)

    """
	Fetch dataset
	"""
    data_set = bfs_dataset(
        data_location=args_seq.data_location,
    ...
    data_loader = DataLoader(
        dataset=data_set, shuffle=False, batch_size=args_final.batch_size
    )
    ...
    if args_final.use_best:
        model.load_state_dict(
            torch.load(args_seq.current_model_save_path + "best_model_sofar")
        )
    else:
        model.load_state_dict(
            torch.load(
                args_seq.current_model_save_path
                + "model_epoch_"
                + str(args_final.Nt_read),
                map_location=torch.device(args_final.device),
            )
        )

    unet1 = Unet3D(
        dim=args_diff.unet_dim,
        cond_images_channels=2,
        memory_efficient=True,
        dim_mults=(1, 2, 4, 8),
    ).to(
        torch.device(args_diff.device)
    )  # mid: mid channel
    image_sizes = 512
    image_width = 512
    ...
    test_final_overall_ensamble(
        args_final, args_seq, args_diff, trainer, model, data_loader
    )
    exit()
    test_final_overall(args_final, args_seq, args_diff, trainer, model, data_loader)
