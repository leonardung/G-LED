...
def test_final_overall_ensamble(
    args_final, args_seq, args_diff, trainer, seq_model, data_loader
):
    down_sampler = torch.nn.Upsample(
        size=args_seq.coarse_dim, mode=args_seq.coarse_mode
    )
    up_sampler = torch.nn.Upsample(size=[512, 512], mode=args_seq.coarse_mode)
            ...
            for iteration, batch in tqdm(enumerate(data_loader)):
                ...
                for i in tqdm(range(local_batch_size)):
                    ...
                    prediction_macro = up_sampler(prediction[0]).reshape(
                        [bsize_here, ntime, num_velocity, 512, 512]
                    )
                    ...
                    pickle.dump(
                        DIC,
                        open(
                            contour_dir + "/" + seq_name + "/DIC" + str(I) + ".npy",
                            "wb",
                        ),
                        protocol=4,
                    )
